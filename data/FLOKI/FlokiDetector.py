...
Read 5-minute FLOKI/USD candles from Coinbase_FLOKIUSD_5min.csv,
label pump / dump events, and export CSVs similar to the PEPE repo:

    floki_pumps_by_candle.csv
    floki_pumps_by_day.csv
    floki_dumps_by_candle.csv
    floki_dumps_by_day.csv
    floki_pump_then_dump_sequences.csv

Run AFTER you have some data:
    python FlokiDetector.py
"""
...
def load_candles() -> pd.DataFrame:
    if not ROOT_CSV.exists():
        raise FileNotFoundError(
            ...
            "Run Coinbase_FLOKIUSD_5min_collector.py first."
        )
...
def export_by_candle_and_day(pumps_df: pd.DataFrame, dumps_df: pd.DataFrame):
    if not pumps_df.empty:
        pumps_df.to_csv(OUT_DIR / "floki_pumps_by_candle.csv", index=False)
        ...
        pumps_by_day.to_csv(OUT_DIR / "floki_pumps_by_day.csv", index=False)

    if not dumps_df.empty:
        dumps_df.to_csv(OUT_DIR / "floki_dumps_by_candle.csv", index=False)
        ...
        dumps_by_day.to_csv(OUT_DIR / "floki_dumps_by_day.csv", index=False)
...
def export_pump_dump_sequences(pumps_df: pd.DataFrame, dumps_df: pd.DataFrame):
    ...
    if sequences:
        ...
        seq_df.to_csv(
            OUT_DIR / "floki_pump_then_dump_sequences.csv", index=False
        )
...
def main():
    ...
    print("Exported FLOKI pump/dump CSVs to the FLOKI/ folder.")
