...
def fetch_candles_with_timeout(product_id="ENA-USD", granularity=300, max_minutes=5):
    """
...
if __name__ == "__main__":
    print("Fetching 5-minute ENA-USD candles (up to 5 minutes runtime)...\n")

    df = fetch_candles_with_timeout(
        product_id="ENA-USD",
        granularity=300,     # 5 minutes
        max_minutes=15        # <= Stop after 5 minutes
    )

    print(f"Fetched {len(df)} rows total.")

    df.to_csv("Coinbase_ENAUSD_5min.csv", index=False)
    print("\n Saved as Coinbase_ENAUSD_5min.csv")
