...
# --- Load & prep ---
# 'PumpAndDumpPredictor\\data\\SOL\\
df = pd.read_csv('Coinbase_SOLUSD_12hr.csv')
...
def generate_event_reports(
    ...
    if not pumps_df.empty:
        pumps_df['date'] = pd.to_datetime(pumps_df['date']) # Ensure 'date' column is datetime type
    ...
    if not dumps_df.empty:
        dumps_df['date'] = pd.to_datetime(dumps_df['date']) # Ensure 'date' column is datetime type
    ...
    # Write files
    pumps_path = 'sol_pumps_by_candle.csv'
    dumps_path = 'sol_dumps_by_candle.csv'
    pumps_day_path = 'sol_pumps_by_day.csv'
    dumps_day_path = 'sol_dumps_by_day.csv'
    seq_path = 'sol_pump_then_dump_sequences.csv'
...
# Save to file
out_path = 'SOL_5min_pump_predictions.csv'
