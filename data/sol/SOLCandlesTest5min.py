...
def fetch_candles_with_timeout(product_id="SOL-USD", granularity=300, max_minutes=5):
    """
...
if __name__ == "__main__":
    print("Fetching 5-minute SOL-USD candles (up to 5 minutes runtime)...\n")

    df = fetch_candles_with_timeout(
        product_id="SOL-USD",
        granularity=300,     # 5 minutes
        max_minutes=(5*12*12)        # <= Stop after 5 minutes
    )

    print(f"Fetched {len(df)} rows total.")

    df.to_csv("Coinbase_SOLUSD_12hr.csv", index=False)
    print("\n Saved as Coinbase_SOLUSD_5min.csv")
