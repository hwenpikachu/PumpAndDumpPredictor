...
# -------------------------------------------------------
# Helper functions for pump/dump detection (same idea as before)
# -------------------------------------------------------
...
# -------------------------------------------------------
# Forecasting model: predict pumps in the *future*
# -------------------------------------------------------

def build_pump_forecast_model(
    price_csv: str = "Coinbase_PEPEUSD_5min.csv",
    pumps_csv: str = "pepe_pumps_by_candle.csv",
    horizon_candles: int = 6,  # forecast next 6 candles = 30 minutes
    out_path: str = "PEPE_5min_pump_forecast_predictions.csv"
):
    ...
    # --- Load detected pump events (ground truth) ---
    ...
    # Mark the actual pump candles
    df_model["is_pump_candle"] = df_model["date"].isin(pump_times).astype(int)

    # --- Create forecast label: will a pump occur in next H candles? ---
    H = horizon_candles
    ...
    # (Optional) keep old-style names for compatibility
    df_model["pump_label_train"] = df_model["pump_forecast_label"]
    df_model["pump_label_B"] = df_model["is_pump_candle"]

    # --- Month information ---
    # We drop timezone info here (warning is harmless), because Period doesn't support tz.
    df_model["month"] = df_model["date"].dt.tz_localize(None).dt.to_period("M")

    # Count positives per month (for the *forecast* label)
    month_pos = df_model.groupby("month")["pump_forecast_label"].sum().sort_index()
    all_months = month_pos.index.to_list()

    print("Per-month forecast positives:")
    for m in all_months:
        print(f"  {m}: {int(month_pos[m])}")

    # --- Choose test month: latest month with at least 1 positive ---
    test_month = None
    ...
    if test_month is not None:
        # Train = all data strictly before that month, Test = that month
        train_mask = df_model["month"] < test_month
        test_mask = df_model["month"] == test_month

        print(f"\nUsing month-based split.")
        print(f"Test month (last month with positives): {test_month}")
        print(f"Train months: {sorted(df_model.loc[train_mask, 'month'].unique())}")
    else:
        # Fallback: chronological 80/20 split if *no* month has any positives
        print("\nNo month has any positive forecast labels — falling back to 80/20 time split.")
        split_idx = int(0.8 * len(df_model))
        train_mask = df_model.index < split_idx
        test_mask = df_model.index >= split_idx

    X = df_model[features]
    y = df_model["pump_forecast_label"].astype(int)
    ...
    if y_train.nunique() >= 2 and y_test.nunique() >= 2:
        ...
        print("\n=== Logistic Regression (test = chosen month or 20% tail) ===")
        print(classification_report(y_test, lr_pred, digits=4))
        print("Confusion matrix:\n", confusion_matrix(y_test, lr_pred))

        df_model.loc[test_mask, "lr_pred"] = lr_pred
        df_model.loc[test_mask, "lr_proba"] = lr_proba
    else:
        print("Not enough class variety for LR (either train or test only has one class).")
    ...
    if y_train.nunique() >= 2 and y_test.nunique() >= 2:
        ...
        print("\n=== Random Forest (test = chosen month or 20% tail) ===")
        print(classification_report(y_test, rf_pred, digits=4))
        print("Confusion matrix:\n", confusion_matrix(y_test, rf_pred))

        feature_importances = dict(zip(features, rf.feature_importances_))
        print("\nRF feature importances:", feature_importances)

        df_model.loc[test_mask, "rf_pred"] = rf_pred
        df_model.loc[test_mask, "rf_proba"] = rf_proba
    else:
        print("Not enough class variety for RF (either train or test only has one class).")

    # --- Save predictions file ---
    out_cols = (
        ["unix", "date", "open", "high", "low", "close", "volume", "month",
         "is_pump_candle", "pump_forecast_label"]
        + features
    ...
    print(f"\nSaved forecast predictions to {out_path}")
...
if __name__ == "__main__":
    # 1) Load raw PEPE candles
    df_raw = pd.read_csv("Coinbase_PEPEUSD_5min.csv")
    df_raw = df_raw.sort_values("unix").reset_index(drop=True)

    # 2) Detect pumps/dumps and write pepe_pumps_by_candle.csv
    generate_event_reports(df_raw, sensitivity='B')

    # 3) Build and evaluate a forecast model:
    #    train on all but last month, test on last month only
    build_pump_forecast_model(
        price_csv="Coinbase_PEPEUSD_5min.csv",
        pumps_csv="pepe_pumps_by_candle.csv",
        horizon_candles=6,
        out_path="PEPE_5min_pump_forecast_predictions.csv"
    )
